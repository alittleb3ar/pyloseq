...
def _to_numeric_frame(
    df: pd.DataFrame, source: Path, cutoff: str | None
) -> pd.DataFrame:
    """Cast every column to float, raising a column-aware error on bad data.

    A bulk ``df.astype(float)`` reports the offending *value* but not which
    OTU column it came from, which makes malformed mothur files hard to
    debug. Converting column by column lets us name the column (and the first
    bad cell) in the error message.
    """
    ...
    for col in df.columns:
        ...
        if bad_mask.any():
            first_label = bad_mask.idxmax()
            first_value = df[col].loc[first_label]
            label_hint = f" (cutoff {cutoff!r})" if cutoff is not None else ""
            raise ValueError(
                f"Non-numeric value {first_value!r} in OTU column {col!r} "
...
def _read_shared(path: Path, cutoff: str | None) -> pd.DataFrame:
    ...
    df.index.name = None
    return _to_numeric_frame(df, source=path, cutoff=cutoff)


def _list_group_to_otu(
    list_path: Path, group_path: Path, cutoff: str | None
) -> pd.DataFrame:
    """Convert .list + .group to a samples-as-rows OTU count DataFrame."""
    # group file: seq_name <tab> sample_name
    group_df = pd.read_csv(
        str(group_path), sep="\t", header=None, names=["seq", "sample"]
    )
    seq_to_sample: dict[str, str] = dict(
        zip(group_df["seq"], group_df["sample"], strict=False)
    )
    ...
    counts: dict[str, dict[str, int]] = {
        s: dict.fromkeys(otu_names, 0) for s in samples
    }
...
def _read_constaxonomy(path: Path) -> pd.DataFrame:
    ...
    for otu_id, row in df.iterrows():
        ...
        parts = [
            p.split("(")[0].strip() for p in raw.rstrip(";").split(";") if p.strip()
        ]
        # Pad to rank length
