...
class TaxTable:
    ...
    def __init__(self, data: pd.DataFrame) -> None:
        ...
        if not data.index.is_unique:
            dups = data.index[data.index.duplicated()].unique().tolist()
            raise ValueError(f"Taxa names must be unique; duplicates: {dups}")
    ...
    def __repr__(self) -> str:
        ...
        if len(self.rank_names) > 3:
            ranks += ", …"
        return (
            f"TaxTable({len(self._df)} taxa × {len(self._df.columns)} ranks [{ranks}])"
        )
