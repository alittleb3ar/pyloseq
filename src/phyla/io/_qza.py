...
def _read_qza_artifact(path: str | Path) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(str(path)) as zf:
        ...
        meta_paths = [n for n in names if n.endswith("/metadata.yaml") and n.count("/") == 1]
        if not meta_paths:
...
def _parse_qiime2_taxonomy(path: Path) -> Any:
    ...
    taxon_col = next((c for c in df.columns if c.lower() in ("taxon", "taxonomy")), None)
    if taxon_col is None:
        raise ValueError(f"No 'Taxon' column found in {path}. Columns: {list(df.columns)}")
